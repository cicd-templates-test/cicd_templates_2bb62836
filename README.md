...
```
dbx execute --cluster-name=<name of interactive cluster> --job=cicd_templates_2bb62836-sample-integration-test
```

For a test on a automated job cluster, use `launch` instead of `execute`:
```
dbx execute --job=cicd_templates_2bb62836-sample-integration-test
```
...
```
dbx launch --job=cicd_templates_2bb62836-sample
```
